...
translate: false
title: "Three Moves"
desc: "Pick fire, water or earth, and see on the screen who won the round and why."
---
# A20: Three Moves
...
**Why bother splitting it?** Because "compare" is the only part you can check without playing. Give it water and fire, and the answer must always be the same. If choosing, comparing and showing were one lump and the wrong name appeared on screen, you would have one question — *"why is it wrong?"* — and three suspects. Split, you can ask "compare" directly and get an answer in a second.
...
```
<button data-move="fire">🔥 fire</button>
<button data-move="water">💧 water</button>
<button data-move="earth">🪨 earth</button>
```
...
`BEATS` is the whole rule of the game written as three pairs. `BEATS.water` is `'fire'`, which reads as "water beats fire". Everything else falls out of it.
...
It also returns a `why`, not just a winner. A game that says "you lose" teaches nothing. A game that says "water beats fire" teaches the rule while you play.
...
Imagine fire simply beat both of the others. What would you pick? Fire. What would everyone pick? Fire. Every round would be fire against fire, forever, and there would be nothing to decide. The game would be over before it started.

Now look at the circle again. Water beats fire. Fire beats earth. Earth beats water. **Every move beats one and loses to one.** There is no move that is just better, so "what should I pick?" has no answer on its own. The only way to answer it is to think about what the *other person* is likely to pick. That is where the game actually lives.
...
![Three big buttons — fire, water, earth — with the result "You win!" and the line "You played water. They played fire. water beats fire."](../static/img/game/a20.png)

Open the page with Live Server and:

1. Press **water**. A result appears with a reason under it.
...
4. In the console, type `compare('earth', 'water')`. You get `{ winner: 'you', why: 'earth beats water' }` without touching the page. That is the point of a pure block.

We ran that last one while building this: `compare('fire', 'water')` came back `{ winner: 'them', why: 'water beats fire' }`, and no button was ever pressed.
