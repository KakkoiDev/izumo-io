...
| 53 | The three moves | ~~fire / water / earth~~ **rock / paper / scissors** | reversal #12, 2026-08-16. Same triangle, no new concept to learn: every child already knows it. It also completes a drift that had already happened — row 52 made the fight elementless, and the animal picker's element label was removed as meaningless, so the elemental layer was vestigial. Deletes `data/type-chart.json`, `elementMultiplier()`, and the `element` field. **A monster is now simply an animal you like the look of** |
| 54 | Creature art | ~~six fantasy creatures~~ **six friendly real animals: lion, boar, elephant, raccoon, deer, ape** | Cyril, on playing it: the previous set had a demon he found disturbing, plus a dragon and a merfolk. Readers are children. Real animals also survive the loss of elements (row 53) in a way "Scorchwing the fire dragon" would not |
| 55 | Installable (PWA) | **Yes — a web app manifest, an icon and a service worker, so it installs to a phone's home screen and opens without a browser bar** | Cyril's call. Cheap on an already-static HTTPS site, and it is the difference between "a link my friend opens once" and "a game on my phone". Teaching-wise it lands in A18, whose subject is already getting the game to other people |
| 56 | Commit–reveal in the game | ~~shipped~~ **Removed. The move is sent directly; A22 becomes an OPTIONAL step** | Cyril, 2026-08-17, after playing it: *"I just want an easy minigame."* Commit–reveal is real cryptography guarding a rock-paper-scissors game between about five friends, and it was most of why the duel screen read as complicated (a folding phase, two fingerprints on screen, a caught-cheating outcome). Cost of removing it, stated in `DESIGN.md` rather than hidden: whoever's move arrives first has shown their hand, so a player who edits the code could answer it. **A22 is kept, not deleted** — the problem is real, the idea is beautiful, and an optional step that says "we chose not to build this, here is what it would have cost" demonstrates the judgement the whole track is about |
