...
status: "optional"
title: "No Peeking"
desc: "Optional: how two players lock in their moves before either is shown, so neither can win by waiting to see the other's."
---
# A22: No Peeking
...
**Needs:** [A20](a20.html). **Gives you:** a round nobody can win by peeking, and an idea used by real voting systems and lotteries.
...
2. **Show both at once** — nobody unfolds until both folded notes are on the table
...
1. Press **scissors**. Two long fingerprints appear. Neither of them tells you anything about either move — that is the point.
...
3. Open the console and type `lastCheck`. You get both moves, both secrets, and `youHonest: true, theyHonest: true` — the proof that the check ran and passed, rather than being skipped.
