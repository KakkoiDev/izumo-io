...
desc: "Instale o VS Code, o Node.js, o Git e um navegador. Verifique pelo terminal. Escreva hello.html e veja rodar."
---
# T01: Configuração do Ambiente

Todo artesão arruma a bancada antes do primeiro corte. Para construir para a web você precisa de quatro ferramentas no computador: um editor para escrever código, um runtime para executar JavaScript fora do navegador, controle de versão para rastrear cada mudança, e um navegador para ver o resultado. Uma tarde de configuração poupa mil frustrações depois. Faça uma vez, esqueça para sempre.
{: .lesson-intro }
...
- **Git** - controle de versão. Rastreia cada mudança que você faz e é o jeito como você compartilha código no GitHub. Vem embutido no macOS e na maioria das distros Linux; Windows precisa de um instalador separado (veja as notas de SO no fim desta aula).
...
Git é o sistema de controle de versão que todo codebase profissional usa e é a língua que o GitHub fala. Você vai usar todo dia a partir do T19.

- **Windows**: baixe o instalador em [git-scm.com](https://git-scm.com/). Aceite os padrões. Ele também traz o Git Bash, que você vai querer como terminal no Windows (muito melhor que o `cmd.exe`).
- **Mac**: o macOS já vem com git, mas o primeiro `git --version` pode pedir pra instalar as Xcode Command Line Tools. Aceite. Alternativamente, `brew install git` dá uma versão mais nova.
- **Linux**: instale pelo gerenciador de pacotes da distro. Exemplo: `sudo apt install git` no Ubuntu/Debian, `sudo dnf install git` no Fedora.

Depois de instalar, diga ao git quem você é. Isso roda uma vez por máquina e assina cada commit que você fizer.
