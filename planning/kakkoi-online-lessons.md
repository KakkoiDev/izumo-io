...
The **second half of the AI track** on school.kakkoi.dev (A09–A28): having learned to use an AI
...
**Prerequisites.** A01–A08 (the agent is installed and they can drive it — this is why we don't write an
install lesson), plus T10–T14 (JS, DOM, persistence) and T19 (git). Stated at the top of A09, not
enforced.
...
| **A09–A28 (this one)** | **one continuous artefact**, built across every lesson; each lesson adds a working feature to the same repo |
...
lesson (A22) is unchanged and still teaches "never trust what another computer sends you" — that lesson
...
- **The safety page is a lesson: A18 "Playing with strangers"**, placed deliberately *before* A19, the
...
## Where we are
Start from tag `a13-end`. Screenshot: before → after.
...
- **The install lesson already exists.** Your original "lesson 1: install Claude and get a
  subscription" is **A01/A02** — done, translated, shipped. The game sub-track starts at planning, and
  we simply don't rewrite what's already there.
...
## 6. The definitive lesson list — 20 lessons, A09–A28
...
### Getting ready (A09–A11)

| # | Title | The one idea | Builds | M |
|---|---|---|---|---|
| A09 | Plan before you prompt | Requirements vs implementation; **a spec is what makes an agent useful** | the student's own `DESIGN.md` | — |
| A10 | Ship an empty page | HTTP vs `file://`, origins, secure contexts, **deploy on day one** | Bun dev server + repo + **live URL already working** | — |
| A11 | Types that never run | Static vs dynamic typing; **something has to strip the types** | `tsc --noEmit` in CI, first typed module | — |
...
| # | Title | The one idea | Builds | M |
|---|---|---|---|---|
| A12 | The game loop | Fixed timestep; **why frame-rate-dependent movement breaks** | 60 Hz loop, delta time | M0 |
| A13 | Moving around | Event-driven vs polling; input state | walking with held keys | M0 |
| A14 | The map | A grid is a data structure; AABB collision; **data-driven design** | JSON tilemap, camera, walls | M0 |
| A15 | Pick your monster | **A sprite sheet is one image you draw rectangles out of** | the ~6 CC0 monsters, a picker, your monster on screen | M0 |
| A16 | Remembering you | Serialisation; **schema versioning** | localStorage save, `v` field | M0 |

### Other people (A17–A23)

| # | Title | The one idea | Builds | M |
|---|---|---|---|---|
| A17 | Who are you? | **Identification vs authentication** | name + element, persisted id | M1 |
| A18 | Playing with strangers | **Nobody is in charge here** — what other people can and cannot see, and what to do if someone is unkind | the in-game safety card, shown on first join | M1 |
| A19 | Two browsers, one wire | NAT, STUN/TURN, signaling vs data plane; **"serverless" is a half-truth** | first real peer connection | M1 |
| A20 | Seeing each other | Full mesh is **O(n²)**; join/leave; eventual consistency | other players visible, online count | M1 |
| A21 | Moving together | Tick rate, **interpolation**; latency hiding | smooth remote players at 10 Hz | M1 |
| A22 | Chat, and hostile input | **Every input is hostile**: schemas, clamps, rate limits, mute | working chat | M2 |

A20 is the payoff lesson of the whole track — the first time a student sees another human being move
...
| # | Title | The one idea | Builds | M |
|---|---|---|---|---|
| A23 | A fight is a state machine | FSM; **pure functions vs I/O** | invite/accept, turn phases | M3 |
| A24 | Fair play with no referee | **Commit–reveal hashing**, WebCrypto; classes of cheating | honest simultaneous moves | M3 |
| A25 | Three buttons | Game theory: **if playing randomly is optimal, there's no game** | Strike/Block/Charge + type triangle | M3 |
| A26 | Someone to fight | **Polymorphism earned**: the battle can't tell human from AI | NPC trainer + townsfolk | M3.5 |

### Sound and reflect (A27–A28)

| # | Title | The one idea | Builds | M |
|---|---|---|---|---|
| A27 | Sound | **Loading a file is not the same as playing it** — browsers refuse to play audio until you interact with the page | 8 CC0 effects + two music loops, muted by default | M4 |

| # | Title | The one idea |
|---|---|---|
| A28 | What we didn't build | Authority, persistence, moderation, matchmaking — what a server buys, what it costs, and **why we cut features on purpose** |

A28 is the honest close, and it doubles as the design lesson: every item in §6.1's cut list is an
...
**A25 "Three buttons" — the main sidebar (~90 words).** Placed right after the three actions are
explained:
...
**A26 "Someone to fight" — a second sidebar (~70 words),** where the NPC prediction tiers are introduced:
...
**A09 "Plan before you prompt"** already carries the design half of this — the Block-pays-nothing reversal
...
### A18 — Playing with strangers (the safety lesson)
...
- One canonical repo, `kakkoi-online`, tagged **`a09-end` … `a28-end`**. Every lesson header says
...
- Students **fork** and work in their own repo, deploying to `their-name.github.io/kakkoi-online/`.
  ...
  the single best demo in the track (A10 sets it up; A19–A20 make it visible).
...
- **Every reversal gets recorded with its reason** in the decision log. Those entries are literally
  A09's teaching material — this document set already contains three (decks → three actions,
...
- **The weekly class *is* the play window.** This quietly solves the empty-world problem (design §2.4)
  without any scheduling machinery: from the week the class reaches A19–A20, the world has a whole room
...
- **Students will be at different lessons.** Someone who ran ahead to A24 is already duelling while the
  class is on A20 — which is *good* (they populate the world and demo what's coming), and is only
  possible because the game is one shared world with no version gate beyond the protocol `v` check.

**Pace:** 20 lessons at one per week is ~5 months, which lines up with the 6-month arc in
...
1. ~~Lesson count?~~ **Settled: 20 (A09–A28).** Shortened by cutting game features (§6.1), not by
...
9. ~~Where does the balance research live?~~ **Settled: two short sidebars in A25/A26, no R essay** —
