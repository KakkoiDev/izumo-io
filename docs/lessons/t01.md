...
desc: "Install VS Code, Node.js, Git, and a browser. Verify with the terminal. Write hello.html and see it run."
---
# T01: Environment Setup

Every craftsman sets up the workbench before the first cut. To build for the web you need four tools on your computer: an editor to write code, a runtime to execute JavaScript outside the browser, version control to track every change, and a browser to view the result. One afternoon of setup saves a thousand frustrations later. Do it once, forget it forever.
{: .lesson-intro }
...
- **Git** - version control. Tracks every change you make and is the way you share code on GitHub. Ships with macOS and most Linux distros; Windows needs a separate installer (see the OS notes at the bottom of this lesson).
...
- **Windows**: download the installer from [git-scm.com](https://git-scm.com/). Accept the defaults. This also gives you Git Bash, which you will want as your terminal on Windows (much nicer than `cmd.exe`).
- **Mac**: macOS bundles git, but `git --version` may prompt to install the Xcode Command Line Tools the first time you run it. Accept. Alternatively, `brew install git` gives you a newer version.
- **Linux**: install via the distro package manager. Example: `sudo apt install git` on Ubuntu/Debian, `sudo dnf install git` on Fedora.
