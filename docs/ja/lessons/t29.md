...
source_sha: "7ac7ed0d94ef"
---
# T29: Reactのステートとエフェクト

ステートはコンポーネントの個人的なノートです。レンダリング間で保持され、更新時に再レンダリングを引き起こすプライベートデータです。エフェクトはコンポーネントのレンダリング後に鳴る目覚まし時計のようなもので、APIやタイマーなどの外部システムと同期できます。
{: .lesson-intro }
