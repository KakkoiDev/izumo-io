...
source_sha: "baad12317c46"
---
# T32: Next.jsのデータとAPI

サーバーコンポーネントのデータ取得は、ウェイターを往復させる代わりにシェフが直接パントリーに行くようなものです。useEffectもローディングスピナーも不要です。コンポーネント自体がasyncで、必要なデータを取得し、サーバーで完全なHTMLをレンダリングします。
{: .lesson-intro }
