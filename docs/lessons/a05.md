...
Describing a file to the AI is like reading a document aloud over the phone: slow and lossy. Instead, hand it the file. In Gemini CLI you do this with `@`:

```
Explain what @notes.txt is about in two sentences.
```

The `@notes.txt` pulls that file's actual contents into your message. Point at the real thing, `@report.md`, `@script.js`, and the AI reads it directly instead of relying on your summary. Because Gemini CLI runs in a folder, it can also see the files in the folder you started it from.
...
1. Create a short text file (some notes, a recipe, anything) in the folder where you run `gemini`.
