...
Toy files are for practice; the point is your real work, an essay, meeting notes, a spreadsheet. On Mac and Linux they are already reachable. On Windows with WSL, your documents live under `/mnt/c/`, so `C:\Users\Alex\Documents` becomes `/mnt/c/Users/Alex/Documents`. `cd` into that folder (put quotes around any path with spaces) and `@` a file like any other.

Plain text drops straight in, `@notes.txt`, `@data.csv`, `@essay.md`. A Word document or PDF is not plain text, so give the AI a text copy: in Word or Google Docs use File then Save As (or Download) as plain text or Markdown, then `@` that copy. For PDFs, or many files at once, a converter like **markitdown** does it in one command (`pip install 'markitdown[all]'`, then `markitdown essay.docx -o essay.md`). Either way you end up with clean text the AI reads perfectly and a copy you can edit.
