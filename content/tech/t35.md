...
What if you could invent your own HTML tag? `<user-card>`, `<rating-stars>`, `<search-box>`. That is Web Components: a browser-native way to build reusable UI Lego bricks with no framework, no build step. Two ingredients today: Custom Elements define the tag, Shadow DOM seals its insides so nothing leaks in or out. {.lesson-intro}
