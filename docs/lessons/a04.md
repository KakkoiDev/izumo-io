...
---
# A04: Install & Authenticate Gemini CLI
...
The `-g` means "install globally" so you can run it from anywhere. If you set up Node with nvm in [A03](a03.html), this just works. If you instead see a long red permission error (`EACCES`), that is the classic reason we recommended nvm, going back and installing Node through nvm fixes it.
...
This uses Google's free tier: no credit card, no API key, generous daily limits. If you ever hit the limit, the tool tells you, just wait a bit or continue tomorrow. (Later, in [A09](a09.html), automated scripts need a different kind of key. Ignore that for now.)
