...
- **Build is a function of source.** Given the current `content/` tree, there is exactly one correct `docs/` tree. Rebuild is deterministic, local, and one command. If the two ever drift, rerun the build.
...
- `.github/workflows/translate-content.yml` - the single workflow that auto-fills translations.
...
<li>The build is a pure function of the content tree. Run it locally, commit the rendered docs/, push. GitHub Pages serves those files directly</li>
