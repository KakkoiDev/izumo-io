...
- `grep pattern file` - find lines matching a pattern. `-r` recurses, `-i` case-insensitive, `-n` line numbers
- `find path -name "*.ext"` - locate files by name, type, size, age
- `sort` / `uniq` - sort lines, collapse duplicates (`uniq -c` counts them)
- `wc -l` - count lines
