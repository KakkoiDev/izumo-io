...
---
# A07: Making & Running Scripts with AI
...
Antigravity CLI has a **headless** mode: give `agy` a `-p` with a prompt and it prints the answer straight to the terminal instead of chatting. That means a script can use it as a step:

```
cat notes.txt | agy -p "Summarize this in three bullets"
```
...
You can schedule scripts to run on their own, `cron` on Mac/Linux, Task Scheduler on Windows. But there is a catch worth knowing: the Google login from A02 expects a human at the browser, so it does **not** cleanly cover a script running while you are asleep. Automating `agy` on a schedule needs non-interactive credentials, a separate and more advanced setup.

You do not need this for the course, everything you run by hand works with the free login. Just know that "run it by myself" and "run it automatically forever" are two different setups.
...
<li>Headless mode (agy -p "...") lets a script call the AI as a step</li>
<li>Running by hand uses the free login; unattended scheduling needs a separate, non-interactive setup</li>
</ul>
</div>
