...
```
onSay((i, id) => {
  if (!Number.isInteger(i) || i < 0 || i >= PHRASES.length) return;
...
3. Now break it on purpose. In the first tab's console, send rubbish by hand: `sendSay(99)`, then `sendSay("Hi!")`, then `sendSay(1.5)`.
4. Look at the second tab. Nothing was drawn. Nothing changed. Type `dropped` in its console and you get `3` — the three bad messages arrived and all three were thrown away. Then move the square and tap a phrase: everything still works.
